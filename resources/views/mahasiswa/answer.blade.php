...
@section('content')
    ...
    <script>
        // Ambil ID pengguna dan survei dari backend
        const userId = {{ auth()->user()->id }};
        const surveyId = {{ $survey->id }};

        // Simpan jawaban ke local storage dengan identifikasi unik
        function saveAnswer(questionId, answer) {
            let answers = JSON.parse(localStorage.getItem(`surveyAnswers_${userId}_${surveyId}`)) || {};
            answers[questionId] = { jawaban: answer };
            localStorage.setItem(`surveyAnswers_${userId}_${surveyId}`, JSON.stringify(answers));
        }
